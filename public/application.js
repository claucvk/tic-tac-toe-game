...
var playerXResults = $('#playerX');
var tiesResults = $('#ties');
var playerOResults = $('#playerO');
var results = $('.results');
...
var mark = function mark(selector, column, columnIndex) {
...
var markerSquare = function markerSquare(turn, columnName, columnIndex) {
  if (columnName === 'a') {
    a[columnIndex] = turn;
...
var onWin = function onWin(player) {
...
var verticalChecking = function verticalChecking() {
  for (var i = 0; i < 3; i++) {
...
var horizontalChecking = function horizontalChecking() {
...
var diagonalLeftChecking = function diagonalLeftChecking() {
...
var diagonalRightChecking = function diagonalRightChecking() {
...
var localStorageSignIn = function localStorageSignIn(dataLocal) {
...
var onCreateGame = function onCreateGame(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.create(data).done(ui.onCreateSuccess).fail(ui.fail);
};
...
var onUpdateGame = function onUpdateGame(index, value, over) {
  ...
  api.updateMoves(index, value, over).done(ui.onUpdateSuccess).fail(ui.fail);
};
...
var onViewGames = function onViewGames(event) {
  event.preventDefault();
  var data = getFormFields(this); //event.target
  api.gameView(data).done(ui.onViewSuccess).fail(ui.fail);
};
...
module.exports = {
  ...
  localStorageSignIn: localStorageSignIn
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var onViewSuccess = function onViewSuccess(data) {
  $('#data-games').empty();
  var hideGamesView = $('#hideGamesView');
